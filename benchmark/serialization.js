...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new flashcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new flashcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = flashcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new flashcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function flashcoreTest() {
      block1 = flashcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('flashcore', flashcoreTest, {maxTime: maxTime});
